...
from paffsteam.confirmation import ConfirmationExecutor
from paffsteam.exceptions import ApiException, SevenDaysHoldException, TooManyRequests
from paffsteam.login import InvalidCredentials, LoginExecutor
from paffsteam.market import SteamMarket
from paffsteam.models import Asset, GameOptions, SteamUrl, TradeOfferState
...
class SteamClient:
    ...
    def _confirm_transaction(self, trade_offer_id: str) -> dict:
        if self.steam_guard["identity_secret"] is None or not self.get_steam_id():
            raise InvalidCredentials(
                "You cannot confirm transaction with only shared_secret passed when initalizing."
            )
